  ...
  if (_src !== '') {
    ...
    for (let i = 0; i < buffers; i++) {
      ...
      audio.onerror = function () {
        console.error(`Error: Failed to load ${_src}`);
      };
      temp.push(audio);
    }
    return temp;
...
function play(sound) {
  // Loop through buffers to find an available one
  for (let i = 0; i < sound.length; i++) {
    let audio = sound[i];
    if (audio.paused || audio.ended) {
      // Reset current time only if the sound isn't playing
      audio.currentTime = 0;
      audio.play().catch((err) => console.error(`Playback error: ${err}`));
      return; // Exit after starting playback
    }
  }
  console.warn('All sound buffers are busy. Consider increasing buffer count.');
}
